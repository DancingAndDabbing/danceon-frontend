...
class ExampleFormModel extends Component {
	...
	componentDidMount() {
		if (this.props.singleExample.title) {
			...
			this.setState({exampleDetails: response, title: this.props.title ? this.props.title : response.title, description: response.description, tag: response.tags, exampleId: response.id})
